...
export default function ControlCenterClient(){
  ...
  const [apiKey,setApiKey]=useState({name:"",expiresAt:""}),[account,setAccount]=useState({email:"",name:"",password:"",globalRole:"viewer"});
  ...
  return <main style={{minHeight:"100vh",background:"#0b0b0b",color:"#f5f5f0",fontFamily:"Inter,system-ui,sans-serif",padding:"28px"}}>
    <div style={{maxWidth:1400,margin:"0 auto"}}>
      ...
      {tab==="management"&&<>
        <Panel title="User accounts"><p>Create real Eventra sign-in accounts. Event roles remain event-scoped, so a user gets access only after you assign their event role.</p><form onSubmit={async e=>{e.preventDefault();try{const r=await fetch("/api/auth/accounts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(account)});const d=await r.json();if(!r.ok)throw new Error(d.error||"Unable to create account");setMessage("Account created: "+d.account.email);setAccount({email:"",name:"",password:"",globalRole:"viewer"})}catch(e){setError(e.message)}}} style={form}><input style={input} type="email" placeholder="Email" value={account.email} onChange={e=>setAccount({...account,email:e.target.value})} required/><input style={input} placeholder="Name" value={account.name} onChange={e=>setAccount({...account,name:e.target.value})}/><input style={input} type="password" minLength="8" placeholder="Temporary password" value={account.password} onChange={e=>setAccount({...account,password:e.target.value})} required/><select style={input} value={account.globalRole} onChange={e=>setAccount({...account,globalRole:e.target.value})}><option value="viewer">Viewer</option><option value="admin">Platform admin</option></select><button style={primary}>Create account</button></form></Panel><Panel title="API keys"><form onSubmit={e=>{e.preventDefault();action({action:"apikey.create",apiKey})}} style={form}><input style={input} placeholder="Key name" value={apiKey.name} onChange={e=>setApiKey({...apiKey,name:e.target.value})} required/><input style={input} type="date" value={apiKey.expiresAt} onChange={e=>setApiKey({...apiKey,expiresAt:e.target.value})}/><button style={primary}>Create API key</button></form>{(data.apiKeys||[]).map(k=><div style={row} key={k.id}><span><b>{k.name}</b><small>{k.active?"Active":"Revoked"} · {k.last_used_at||"never used"}</small></span><button style={danger} onClick={()=>action({action:"apikey.revoke",id:k.id})}>Revoke</button></div>)}</Panel>
