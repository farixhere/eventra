...
export async function POST(request){
 try{
  ...
  const sql=getDb(),u=await parseUserToken(request.cookies.get("eventra_session")?.value);
  ...
  const ranked=b.participantId
   ? await sql`SELECT id,RANK() OVER (ORDER BY total_score DESC) AS position FROM results WHERE programme_id=${b.programmeId} AND participant_id IS NOT NULL AND verification_status='verified' ORDER BY total_score DESC,id`
   : await sql`SELECT id,RANK() OVER (ORDER BY total_score DESC) AS position FROM results WHERE programme_id=${b.programmeId} AND team_id IS NOT NULL AND verification_status='verified' ORDER BY total_score DESC,id`;
  for(const row of ranked){
   const rank=Number(row.position);
   await sql`UPDATE results SET position=${rank},points=${rank===1?5:rank===2?3:rank===3?1:0} WHERE id=${row.id}`;
  }
