...
export async function GET(){
 try{
  ...
  const hasCols=(table, names)=>names.every(name=>cols.some(x=>x.table_name===table&&x.column_name===name));
  const signatures={
   migration005:present.has("event_roles")&&present.has("judge_assignments")&&present.has("certificate_verifications"),
   migration006:present.has("users")&&present.has("roles")&&present.has("permissions")&&present.has("user_roles")&&present.has("sessions")&&hasCols("events",["owner_user_id"])&&hasCols("audit_logs",["actor_user_id","reason","request_id","ip_address"]),
   migration007:hasCols("events",["timezone","settings","registration_settings"])&&hasCols("programmes",["rules_config","scoring_config","registration_limit","team_size_min","team_size_max","skill_id"])&&present.has("programme_criteria")&&present.has("judge_scores")&&present.has("result_corrections")&&present.has("festival_settings")&&present.has("api_keys")&&hasCols("results",["score_breakdown","judge_scores","published_by","corrected_at","correction_reason"]),
