...
export async function POST(request){
  try{
    ...
    if(action==="results.aggregate"){
      ...
      const grouped=await sql`SELECT COALESCE(participant_id::text,team_id::text) AS entry_key,(array_agg(participant_id) FILTER (WHERE participant_id IS NOT NULL))[1] AS participant_id,(array_agg(team_id) FILTER (WHERE team_id IS NOT NULL))[1] AS team_id,ROUND(AVG(total_score),2) AS total_score,COUNT(*)::int AS judges FROM judge_scores WHERE event_id=${eventId} AND programme_id=${b.programmeId} AND status='submitted' GROUP BY COALESCE(participant_id::text,team_id::text) ORDER BY AVG(total_score) DESC`;
