...
export async function POST(request) {
  try {
    ...
    let user = await getOrBootstrapAdmin(email,password);
    if (!user) {
      const rows = await sql.unsafe(
        `SELECT u.id,u.email,u.display_name,u.active,u.password_hash,
          ...
          WHERE lower(u.email)=lower($1) GROUP BY u.id LIMIT 1`, [email]
      );
      user = rows[0] || null;
    }
    ...
    const response = Response.json({ok:true,user:{id:user.id,email:user.email,name:user.display_name,globalRole,roles:user.roles||[]}},
