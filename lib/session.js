...
export async function getSessionUser(token) {
  ...
  const configuredAdminEmail = String(process.env.EVENTRA_ADMIN_EMAIL || "owner@eventra.local").trim().toLowerCase();
  const roles = rows[0].roles || [];
  const isBootstrapAdmin = String(rows[0].email || "").toLowerCase() === configuredAdminEmail;
  const effectiveRoles = isBootstrapAdmin && !roles.includes("admin") ? ["admin", ...roles] : roles;
  return { id:rows[0].id,email:rows[0].email,name:rows[0].display_name,
    globalRole:effectiveRoles.includes("admin")?"admin":(effectiveRoles[0]||null),
    roles:effectiveRoles,sessionId:rows[0].session_id };
}
