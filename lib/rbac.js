...
const perms={
  admin:new Set(["*"]),
  coordinator:new Set(["read","manage"]),
...
function required(path,method,body){
  ...
  if(path.includes("/judge-assignments")) return "manage";
...
export async function authorizeRequest(request){
  ...
  if(path.includes("/results")&&request.method!=="GET"&&role!=="coordinator")
    return{ok:false,status:403,error:"Only a coordinator or admin can manage results"};

  if(path.includes("/result-corrections")&&role!=="coordinator")
    return{ok:false,status:403,error:"Only a coordinator or admin can correct results"};

  if(path.includes("/judge-assignments")&&role!=="coordinator")
    return{ok:false,status:403,error:"Only a coordinator or admin can manage judge assignments"};
