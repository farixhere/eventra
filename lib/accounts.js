...
async function findUserByEmail(sql, email) {
  try {
    ...
    // Authentication must still work while RBAC migrations are being repaired.
    // The bootstrap admin identity is promoted by email in this module/session layer.
    console.error("auth.find-user.rbac-fallback", error);
    const rows = await sql.unsafe(
      `SELECT id,email,display_name,active,password_hash
         FROM users
        WHERE lower(email)=lower($1)
        LIMIT 1`,
      [email]
    );
    return rows[0] ? { ...rows[0], roles: [] } : null;
  }
}
...
export async function getOrBootstrapAdmin(email, password) {
  ...
  try {
    ...
    console.error("auth.bootstrap.user-count", error);
    throw error;
  }
