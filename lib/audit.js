...
export async function auditLog(request,{action,eventId=null,entityType=null,entityId=null,changes=null,reason=null}={}){
  try{
    ...
    await sql`INSERT INTO audit_logs(event_id,action,entity_type,entity_id,changes,actor_user_id,reason,request_id,ip_address)
      VALUES(${eventId||null},${action},${entityType},${entityId||null},${changes?JSON.stringify(changes):null}::jsonb,${null},${reason||null},${request?.headers?.get("x-request-id")||null},${request?.headers?.get("x-forwarded-for")?.split(",")[0]?.trim()||null})`;
